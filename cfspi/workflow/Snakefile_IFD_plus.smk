...
rule fff_kraken2_standardDB:
    ...
    shell:
        """
        kraken2 --confidence {params.threshold} --db {params.database_dir}RS_minusT2T --threads {threads} \
	--report-zero-counts --report-minimizer-data \
	--output {output.k2_chm13_grch38_out} --report {output.k2_chm13_grch38_rep} --paired {input.R1_chm13_grch38} {input.R2_chm13_grch38} ; 
        grep -v "^U"  {output.k2_chm13_grch38_out} > {output.k2_chm13_grch38_clf};

        kraken2 --confidence {params.threshold} --db {params.database_dir}RS_minusT2T --threads {threads} \
	--report-zero-counts --report-minimizer-data \
	--output {output.k2_chm13_out} --report {output.k2_chm13_rep} --paired {input.R1_chm13} {input.R2_chm13} ; 
        grep -v "^U"  {output.k2_chm13_out} > {output.k2_chm13_clf};

        kraken2 --confidence {params.threshold} --db {params.database_dir}RS_minusT2T --threads {threads} \
	--report-zero-counts --report-minimizer-data \
	--output {output.k2_grch38_out} --report {output.k2_grch38_rep} --paired {input.R1_grch38} {input.R2_grch38} ; 
